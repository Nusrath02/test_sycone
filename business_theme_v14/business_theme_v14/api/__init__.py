...
def get_org_chart_data(company, department="", branch=""):
    ...
    for e in employees:
        result.append({
            "id": e.name, "name": e.employee_name,
            "designation": e.designation or "", "department": e.department or "",
            "branch": e.branch or "", "reports_to": e.reports_to or "",
            "image": e.image or "",
        })
    ...
    if department or branch:
        ...
        for eid in extras:
            em = emp_map[eid]
            visible.append({
                "id": em.name, "name": em.employee_name,
                "designation": em.designation or "", "department": em.department or "",
                "branch": em.branch or "", "reports_to": em.reports_to or "",
                "image": em.image or "",
            })
    return {"employees": visible, "departments": all_depts, "branches": all_branches}
