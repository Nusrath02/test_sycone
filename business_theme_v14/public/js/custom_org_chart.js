...
function paletteFor(department) {
    if (!department) return PALETTE[0];
    return PALETTE[hashStr(department) % PALETTE.length];
}
    ...
    frappe.call({
        ...
        callback: function (r) {
            if (r && r.message) {
                ...
                var depts = msg.departments && msg.departments.length ? msg.departments :
                    _allEmps.map(function (e) { return e.department; })
                             .filter(function (v, i, a) { return v && a.indexOf(v) === i; }).sort();
                var branches = msg.branches && msg.branches.length ? msg.branches :
...
function nodeH(node, depth) {
    var d = node.d, kids = node.ch, c = paletteFor(d.department);
