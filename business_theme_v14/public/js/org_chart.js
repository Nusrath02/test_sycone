...
function itc_setup(wrapper) {
    ...
    if (!wrapper._itc_filters_done) {
        ...
        $(page.body || page.main).html('<div class="itc-page" id="itc-page"><div class="itc-empty">Loading...</div></div>');
...
function doLoad(page) {
    ...
    if (!co) {
        $("#itc-page").html('<div class="itc-empty">Select a company</div>');
        return;
    }
    ...
    cm = {}; ci = 0;
    $("#itc-page").html('<div class="itc-empty">Loading...</div>');
    ...
    frappe.call({
        ...
        error: function () {
            $("#itc-page").html('<div class="itc-empty">Error loading data</div>');
        }
    });
}
...
function doRender(emps) {
    if (!emps || !emps.length) {
        $("#itc-page").html('<div class="itc-empty">No employees found</div>');
        return;
    }
...
function nodeH(node) {
    ...
    var av = d.image
        ? '<div class="itc-av"><img src="' + d.image + '"></div>'
        : '<div class="itc-av" style="background:' + col + '">' + abr(d.name) + '</div>';
