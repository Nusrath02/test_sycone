...
(function () {
  console.log("[ITC-ORG] itc_org_chart.js loaded");
  ...
  function injectFilters(depts, branches) {
    ...
    function makeWrap(wrapId, selId, labelText, items) {
      ...
      sel.addEventListener("change", function () { renderTree(); });
      wrap.appendChild(lbl);
      wrap.appendChild(sel);
      return wrap;
    }

    container.appendChild(makeWrap("itc-dept-wrap", "itc-dept-sel", "Department", depts));
    container.appendChild(makeWrap("itc-branch-wrap", "itc-branch-sel", "Branch", branches));
  }
    ...
    frappe.call({
      ...
      callback: function (r) {
        if (r && r.message) {
          ...
          var depts = msg.departments && msg.departments.length
            ? msg.departments
            : _allEmps.map(function (e) { return e.department; })
                .filter(function (v, i, a) { return v && a.indexOf(v) === i; }).sort();
          var branches = msg.branches && msg.branches.length
            ? msg.branches
  ...
  function nodeH(node) {
    ...
    // color comes from the API (set server-side in org_chart.py)
    var c = d.color || { bg: "#dbeafe", bd: "#2563eb", tx: "#1e40af" };
    ...
    if (parts.length)
      h += '<div class="org-card-info">' + esc(parts.join(" \u00b7 ")) + "</div>";
    h += "</div>";
    if (kids.length > 0) {
